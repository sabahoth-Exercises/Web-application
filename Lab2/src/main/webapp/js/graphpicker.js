(function () {
    window.GraphPicker = function(canvas) {
        ...
        function render() {
            ...
            ctx.fillRect(0.5*width, 0.3*height, 0.4*width, 0.2*height);
            //triangle
            ctx.beginPath();
            ctx.mv(.1, .5);
            ctx.ln(.5, .3);
            ctx.ln(.5, .5);
            //arc
            ctx.arc(0.5*width, 0.5*height, .2*(width+height)/2, 0.5*Math.PI, Math.PI);
            ctx.fill();
            ...
            ctx.mv(0, .5);
            ctx.ln(1, .5);//x axe
            ctx.ln(.97, .48);//upper trait for arrow
            ctx.mv(1, .5);
            ctx.ln(.97, .52);//lower trait for arrow
            ctx.mv(.1, .49);//mark on x from the left
            ctx.ln(.1, .51);// 1st
            ctx.mv(.3, .49);
            ctx.ln(.3, .51);// 2nd
            ctx.mv(.7, .49);
            ...
            ctx.ln(.48, .03);
            ctx.mv(.5, 0);
            ctx.ln(.52, .03);// r/2 mark on y
            ctx.mv(.49, .9);
            ctx.ln(.51, .9);// -r mark on y
            ctx.mv(.49, .7);
            ctx.ln(.51, .7);// -r/2 mark on y
            ctx.mv(.49,.3);
            ctx.ln(.51,.3);
            ctx.mv(.49, .1);
            ctx.ln(.51, .1);// r mark
